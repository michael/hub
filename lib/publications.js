...
var BY_DOCUMENT_SQL = 'SELECT * FROM Publications WHERE document = $1',
    ...
    DOCUMENTS_BY_NETWORK_SQL = 'SELECT d.id as id, v.created_at as created_at, d.creator as creator, v.data as data FROM documents d, versions v, publications p WHERE v.document = d.id AND v.version = d.latest_version AND p.network = $1 AND p.document=d.id ORDER BY v.created_at DESC;',
