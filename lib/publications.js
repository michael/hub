...
// Create Publication
publications.create = function(network, document, creator, data, callback) {
  ...
  documents.incrementVersion(document, function(err, version) {
    ...
    versions.create(document, version, creator, data, function(err) {
      ...
      // 2. create publication entry which connects the document and network
      db.query(INSERT_PUBLICATION_SQL, [network, document, version, creator, 'active'], function (err, results) {
