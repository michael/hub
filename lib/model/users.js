...
users.findByLogin = function (login, cb) {
  db.query(AUTHENTICATE_SQL, [login], db.one(cb, 'No user found for '+login));
};

// Gosh, something better?
users.validate = function (email, username, name, password, cb) {
...
users.createSafe = function (user, cb) {
  users.validate(user.email, user.username, user.name, user.password, function (err, valid) {
    if (err) cb(err, null);
    else users.insert(user, cb);
  });
};
