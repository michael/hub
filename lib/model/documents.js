...
documents.get = function(document, cb) {
  db.query(DOCUMENT_BY_ID, [document], db.one(cb, "Unknown document"));
};
...
documents.delete = function(document, cb) {
  db.query(DELETE_DOCUMENT_SQL, [document.id], function (err, results) {
    if (err) cb(new errors.InternalServerError("SQL error: could not delete document."));
    else cb(null);
  });
};
...
documents.isCreator = function(user, document, cb) {
  documents.get(document, function(err, doc) {
    if(err) cb(new errors.NotFound("Unknown document " + document));
    else if(doc.creator !== user) cb(errors.Forbidden("User is not creator."));
    else cb(null);
  });
};
