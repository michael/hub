...
HubStore.__prototype__ = function() {
    ...
    this.create = function (docId, options, cb) {
      ...
      documents.get(docId, function(err, alreadyExists) {
        if (alreadyExists) {
          ...
          var document = {id: docId, creator: this.username};
    ...
    this.list = function(cb) {
      ...
      function _getCollaborations(cb) {
        self.getCollaborations(function(err, data) {
          collaborations = data;
...
var api = {
  ...
  "blobs.get": {
    ...
    method: withDoc(function(args, doc, cb) {
      ...
      new HubStore(doc.creator).blobs.get(doc.id, args.blob, cb);
    })
  },
  "blobs.find": {
    ensure: global.api.isCollaboratorOrCreator,
    method: withDoc(function(args, doc, cb) {
      ...
      new HubStore(doc.creator).blobs.list(doc.id, cb);
    })
  },
  "blobs.create": {
    ensure: global.api.isCollaboratorOrCreator,
    method: withDoc(function(args, doc, cb) {
      ...
      new HubStore(doc.creator).blobs.create(doc.id, args.blob, args.data, cb);
    })
  },
}
