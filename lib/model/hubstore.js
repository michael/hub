...
var api = {
  ...
  "getChanges": {
    method: function(args, cb) {
      ...
      if (args.track != Store.MAIN_TRACK) {
        global.api.isCollaboratorOrCreator({document: args.track}, {username: args.username}, function(err) {
          if (err) return cb(err);
          documents.get(args.track, function(err, doc) {
            if (err) return cb(err);
            new HubStore(doc.creator).getChanges(doc.id, args.ids, cb);
          });
        });
      } else new HubStore(args.username).getChanges(args.track, args.ids, cb);
    }
  },
  "getIndex": {
    method: function(args, cb) {
      //console.log("hubstore getIndex", args);
      if (args.track != Store.MAIN_TRACK) {
        global.api.isCollaboratorOrCreator({document: args.track}, {username: args.username}, function(err) {
          if (err) return cb(err);
          documents.get(args.track, function(err, doc) {
            if (err) return cb(err);
            new HubStore(doc.creator).getIndex(doc.id, cb);
          });
        });
      } else new HubStore(args.username).getIndex(args.track, cb);
    }
  },
  "applyChange": {
    method: function(args, cb) {
      ...
      if (args.track != Store.MAIN_TRACK) {
        global.api.isCollaboratorOrCreator({document: args.track}, {username: args.username}, function(err) {
          ...
          documents.get(args.track, function(err, doc) {
            if (err) return cb(err);
            new HubStore(doc.creator).applyCommand(doc.id, args.command, cb);
          });
        });
