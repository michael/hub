...
function createDocumentEntry(creator, docId, cb) {
...
documents.create = function (creator, docId, meta, callback) {
  ...
  documents.getDocumentById(docId, function(err, alreadyExists) {
    if (alreadyExists) callback(new errors.WrongValue("Document already exists: " + creator + ", " + docId));
    else {
      createDocumentEntry(creator, docId, function(err) {
