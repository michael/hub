...
documents.getDocumentById = function(docId, cb) {
  console.log("documents.getDocumentById("+docId+")");
  db.query(DOCUMENT_BY_ID, [docId], db.one(cb, 'Document not found'));
}
...
documents.getDocumentAccess = function(username, docId, permission, callback) {
  ...
  documents.getDocumentById(docId, function(err, doc) {
    console.log("Result of getDocumentById: ", JSON.stringify(err), JSON.stringify(doc));
    if (doc) collaborators.listCollaborators (docId, function(err, result) {
      ...
      if (isCreator || (permits[permission] && permits[permission](username))) {
        console.log("Access permitted: ", permission, username, docId);
        callback(null, doc);
      } else {
        console.log("Access denied: ", permission, username, docId);
        callback("Operation not permitted: " + permission + " for user " + username);
      }
    });
    else callback("Could not retrieve document.");
  });
};
...
documents.create = function (creator, document, meta, callback) {
  ...
  // this is only called after a document has been created in
  // the postgres backend but fails to be created in the user's redis store.
  // TODO: should use sql transaction for that
  function rollback(err) {
    console.log("documents.create: rolling back changes.");
    documents.delete(docStub, function(err, result) {});
    callback(err);
  }

  // Note: assuming that the document does not exist
  documents.getDocumentById(document, function(err, alreadyExists) {
    if (alreadyExists) callback("Document already exists: " + creator + ", " + document , docStub);
    else {
...
documents.update = function(doc, commits, meta, refs, callback) {
  console.log("documents.update", doc);
  ...
  collaborators.listCollaborations(username, function(err, collaborations) {
    ...
    function next(index) {
      if(index < collaborations.length) {
        ...
        console.log("User collaborations: ", result);
        callback(null, result);
      }
    }
    next(0);
