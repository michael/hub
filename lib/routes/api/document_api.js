...
module.exports = function(app) {
  ...
  app.get('/documents/:document/blobs/:blob',
    ...
    function(req, res, next) {
      ...
      global.api.execute("hubstore", "blobs.get", args, req.user, function(err, blob) {
        if (err) return res.json(err);
        else res.json(blob);
      });
    });
  ...
  app.get('/documents/:document/blobs',
    ...
    function(req, res, next) {
      ...
      global.api.execute("hubstore", "blobs.find", args, req.user, function(err, blobIds) {
        if (err) return res.json(err);
  ...
  app.post('/documents/:document/blobs/:blob',
    ...
    function(req, res, next) {
      ...
      global.api.execute("hubstore", "blobs.create", args, req.user, function(err, blob) {
        if (err) return res.json(err);
        else res.json({"status": "ok"});
      });
    });
