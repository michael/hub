...
module.exports = function(app) {
  ...
  app.delete('/documents/:document',
    ...
    function(req, res, next) {
      ...
      var hubstore = new HubStore({username: username});
      hubstore.getDocumentAccess(username, document, 'delete', function(err, doc) {
        if (err) res.json(err.status, err);
        else hubstore.delete(doc, function(err, result) {
          if (err) next(err);
  ...
  app.put('/documents/:document',
    ...
    function(req, res, next) {
      ...
      hubstore.getDocumentAccess(username, document, 'write', function(err, doc) {
        if(err) next(err);
        else hubstore.update(doc, commits, meta, refs, function(err, results) {
          if (err) next(err);
          else res.json({"status": "ok"});
        });
      });
  ...
  app.get('/documents/:document',
    ...
    function(req, res, next) {
      ...
      hubstore.getDocumentAccess(username, document, 'read', function(err, doc) {
        if (err) next(err);
        else hubstore.get(doc, function(err, fullDoc) {
          if (err) next(err);
  ...
  app.get('/documents/:document/commits',
    ...
    function(req, res, next) {
      ...
      // TODO: refactor this: HubStore should automatically dispatch to the correct store
      var hubstore = new HubStore({username: username});

      hubstore.getDocumentAccess(username, document, 'read', function(err, doc) {
        if (err) return next(err);

        hubstore.commits(doc, startCommit, function(err, result) {
          res.json(result);
        });
      });
    });
  ...
  app.get('/documents',
    ...
    function(req, res, next) {
      ...
      hubstore.list(username, function(err, docs) {
        if (err) next(err);
        else res.json(docs);
      });
    });
  ...
  app.post('/documents',
    ...
    function(req, res, next) {
      ...
      hubstore.create(username, document, meta, function(err, doc) {
        if (err) next(err);
        else res.json(doc);
      });
    });
