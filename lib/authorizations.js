...
authorizations.create = function (user_uuid, application_uuid, scopes, callback) {
  var uuid = db.uuid();
  var token = db.uuid();
  db.query(INSERT_SQL, [uuid, user_uuid, application_uuid, token, scopes], function (err) {
...
authorizations.findOrCreate = function (user_uuid, application_uuid, scopes, callback) {
  authorizations.findByUserAndApplication(user_uuid, application_uuid, function (err, application) {
    if (err instanceof errors.NoRecordFound) authorizations.create(user_uuid, application_uuid, scopes, function (err, uuid) {
      if (err) callback(err, null);
