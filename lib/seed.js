...
var Seed = function(seed) {
  ...
  // var seedRemoteStore = util.async.each({
  //   selector: function() { return seed.remote; },
  //   iterator: function(storeSeed, cb) {
  //     documents.seed(storeSeed, cb);
  //   }
  // });
...
Seed.read = function(env, name, cb) {
  ...
  if (env === "production") {
    res.hub = JSON.parse(fs.readFileSync('./data/seed.json','utf8'));
    cb(null, seed);
  } else {
    // read the config and pull in dependencies
    util.loadSeed(name, function(err, data) {
