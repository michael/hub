...
DocumentRenderer.prototype.nodes = function() {
  ...
  return _.map(doc.lists.content, function(n) {return doc.nodes[n]; });
};
...
DocumentRenderer.prototype.render = function() {  
  ...
  var html = '';
  if (properties.cover_medium) html += '<div class="cover"><img src="/api/v1/documents/'+content.id+'/image/'+properties.cover_medium.replace(':', '_')+'"/></div>';
  ...
  _.each(this.nodes(), function(node) {
    if (node.type === "heading") {
      ...
      html += '<img src="/api/v1/documents/'+content.id+'/image/'+node.medium.replace(':', '_')+'"/>';
